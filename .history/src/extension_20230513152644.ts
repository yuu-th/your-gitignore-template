		...
		vscode.window
		  ...
		  .then((selected) => {
			...
			if (selected) {
			  ...
			  const selectedGitignorePath = path.join(
				userGitignoreDir,
				selected.label
			  );
			  // 選択されたgitignoreファイルの内容を読み込む
			  const selectedGitignoreContent = fs.readFileSync(
				selectedGitignorePath,
				"utf-8"
			  );
			  ...
			  if (fs.existsSync(workspaceGitignorePath)) {
				...
				vscode.window
                .showWarningMessage(
					"A .gitignore file already exists in the workspace folder. Do you want to overwrite it?",
					"Yes",
					"No"
				  )
				  .then((answer) => {
					...
					if (answer === "Yes") {
					  ...
					  fs.writeFileSync(
						...
						selectedGitignoreContent
					  );
					  vscode.window.showInformationMessage(
						"The .gitignore file has been overwritten."
					  );
					} else if (answer === "No") {
					  // 上書きしない場合は何もしない
					  vscode.window.showInformationMessage(
						"The .gitignore file has not been changed."
					  );
					}
				  });
			  } else {
				...
				vscode.window.showInformationMessage(
				  "A new .gitignore file has been created."
				);
			  }
			}
		  });
